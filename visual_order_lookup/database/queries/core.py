...
def _parse_date_field(value: Union[str, date, datetime, None]) -> Optional[date]:
    """Parse a date field that may be stored as string, date, or datetime.

    Args:
        value: Date value from database (string, date, datetime, or None)
...
def search_by_job_number(cursor: pyodbc.Cursor, job_number: str) -> Optional[OrderHeader]:
    ...
    query = """
        SELECT
            ...
            co.USER_10 AS quote_id,
            co.USER_6 AS revision_date,
            co.USER_7 AS revision_number,
            c.ID AS customer_id,
            c.NAME AS customer_name,
    ...
    try:
        ...
        order = OrderHeader(
            ...
            revision_number=str(row.revision_number).strip() if row.revision_number and str(row.revision_number).strip() else None,
            revision_date=_parse_date_field(row.revision_date),
            ...
            factory_acceptance_date_firmed=row.promise_date.date() if row.promise_date and hasattr(row.promise_date, 'date') else (row.promise_date if row.promise_date else None),
